...
namespace u23717662_HW01.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult SOSBooking()
        {
            ...
            Booking booking = new Booking
            {
                ...
                DriverID = random.Next(1, DriverRepository.Drivers.Count + 1), // Randomly assign a driver
                VehicleID = random.Next(1, VehicleRepository.Vehicles.Count + 1), // Randomly assign a vehicle
                ReasonID = ReasonRepository.Reasons.FirstOrDefault()?.ReasonID ?? 1, // Default to first reason if none exist
                PickupAddress = "Emergency Location",
                ServiceID = 1 // Default service ID, can be adjusted as needed
            };
        ...
        public ActionResult CreateBooking(Booking booking)
        {
            ...
            {
                // Repopulate dropdowns and other ViewBag items
                ViewBag.ServiceID = booking.ServiceID;
                ViewBag.Vehicles = VehicleRepository.Vehicles.Where(v => v.ServiceID == booking.ServiceID).ToList();
                ViewBag.Drivers = DriverRepository.Drivers.Where(d => d.ServiceID == booking.ServiceID).ToList();
                ViewBag.Reasons = ReasonRepository.Reasons;

                return View("Book", booking); // return the same view with errors
            }
